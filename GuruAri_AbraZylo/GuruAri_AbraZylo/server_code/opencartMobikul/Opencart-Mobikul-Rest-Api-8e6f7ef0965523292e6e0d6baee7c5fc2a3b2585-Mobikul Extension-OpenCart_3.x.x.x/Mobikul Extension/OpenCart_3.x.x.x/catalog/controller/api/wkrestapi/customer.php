    ...
    protected function executeCurl($device_id, $title, $desc, $comment) {
		...
		if($accessToken){
            ...
            $data = array(
                ...
                'mutable_content' => $comment ? 'true' : 'false'
            );
  ...
  /**
        ...
        if (!isset($post['wk_token']) || !$this->checkSession($post['wk_token'])) {
            ...
            if (!$getTotalOrders) {
                ...
                foreach ($getOrders as $order) {
                    ...
                    // Get first product image for this order
                    $order_image = '';
                    $order_products = $this->model_account_order->getOrderProducts($order['order_id']);
                    if (!empty($order_products)) {
                        $this->load->model('catalog/product');
                        $this->load->model('tool/image');
                        $product_info = $this->model_catalog_product->getProduct($order_products[0]['product_id']);
                        if (!empty($product_info['image'])) {
                            $order_image = $this->model_tool_image->resize($product_info['image'], 200, 200);
                        }
                    }
    ...
    public function getOrderInfo()
    {
        ...
        if (!isset($post['wk_token']) || !$this->checkSession($post['wk_token'])) {
            ...
            $date_added = isset($order_info['date_added']) 
    ...
    */
    ...
    /**
     * Google Sign-In — find or create a customer by Google account, then log them in.
     *
     * POST params: wk_token, google_id, email, firstname, lastname,
     *              android_device_id (optional), ios_device_id (optional)
     *
     * Returns JSON matching LoginModel:
     *   {"error": 0, "customer_id": X, "firstname": "...", "lastname": "...", "email": "..."}
     */
    public function googleLogin()
    {
        ...
        // Accept data in JSON / raw format
        $raw_data = json_decode(file_get_contents("php://input"), true);
        if ($raw_data) {
            foreach ($raw_data as $key => $value) {
                $post[$key] = $value;
        ...
        // Get wk_token from header if not in POST body
        if (isset(getallheaders()['wk_token'])) {
            $post['wk_token'] = getallheaders()['wk_token'];
        } elseif (isset(getallheaders()['Wk-Token'])) {
            $post['wk_token'] = getallheaders()['Wk-Token'];
        ...
        if (empty($post['email']) || empty($post['google_id'])) {
            $this->response->addHeader('Content-Type: application/json');
            $this->response->setOutput(json_encode(array('error' => 1, 'message' => 'Missing required Google sign-in parameters.')));
            return;
        }

        $email     = trim($post['email']);
        $google_id = trim($post['google_id']);
        $firstname = isset($post['firstname']) ? trim($post['firstname']) : '';
        $lastname  = isset($post['lastname'])  ? trim($post['lastname'])  : '';
        $android_device_id = isset($post['android_device_id']) ? $post['android_device_id'] : '';
        $ios_device_id     = isset($post['ios_device_id'])     ? $post['ios_device_id']     : '';
        ...
        if ($total > 0) {
            // Customer already exists — log them in (bypass password check)
            $login_result = $this->customer->login($email, '', true);
        } else {
            // Create a new customer account
            $random_password = substr(md5(uniqid(mt_rand(), true)), 0, 16);
            ...
            // Log in the newly created customer
            $login_result = $this->customer->login($email, '', true);
        }
        ...
        if ($login_result) {
            ...
            if ($android_device_id || $ios_device_id) {
                $this->load->model('wkrestapi/customer');
                $this->model_wkrestapi_customer->registerDeviceToken($android_device_id, $ios_device_id);
            }
            ...
            $return_array = array(
                'error'   => 1,
                'message' => 'Google sign-in failed. Unable to authenticate customer.',
            );
        }
